...
if [! -d /mnt/disk-for-data ]; then
    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Mounting external data disk"
    ...
    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Creating data folders"
    ...
    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Installing docker subsystem"
    sudo apt update
    ...
    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Pulling mysql-server:8.0 docker image"
    sudo docker image pull mysql/mysql-server:8.0
    echo "--------------------------------------------------------"
    echo

    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Staarting mysql-server:8.0 docker container"
    ...
    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Cloning github repository"
    sudo mkdir /soft
    ...
    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Installing python dependencies"
    ...
    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Restaring rdbms (mysql) docker container"
    sudo docker restart mysql
    echo "--------------------------------------------------------"
    echo

    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Installing mysql client tool"
    sudo apt install --yes mysql-client
    echo "------------------------------------------------------------"
    echo

    echo "------------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "Creating a swap file (2GB)"
    sudo mkdir -v /var/cache/swap
    cd /var/cache/swap
    sudo dd if=/dev/zero of=swapfile bs=1K count=2M

    echo $(date "+%A %W %Y, %X UTC:   ") "Enabling swapping"
    sudo chmod 600 swapfile
    sudo mkswap swapfile
    sudo swapon swapfile

    echo $(date "+%A %W %Y, %X UTC:   ") "Adding record to /etc/fstab for automounting swap file"
    echo "/var/cache/swap/swapfile none swap sw 0 0" | sudo tee -a /etc/fstab
    echo "------------------------------------------------------------"
    echo
else
    echo "--------------------------------------------------------"
    echo $(date "+%A %W %Y, %X UTC:   ") "The automation script had been executed previously"
    echo "--------------------------------------------------------"
    echo
fi

echo "--------------------------------------------------------"
echo $(date "+%A %W %Y, %X UTC:   ") "Waiting for everything to be started and mounted"
# hope 1m is enough...
sleep 60
echo "------------------------------------------------------------"
echo
...
echo "------------------------------------------------------------"
echo $(date "+%A %W %Y, %X UTC:   ") "Starting cards_finder_cars_com.py"
